...
const BOOL ServiceManager::InitialProfile()
{
	...
	printf("SERVER NAME : %s\n", m_ServerInfo.szServerName);
	printf("SERVER IP : %s\n", m_ServerInfo.szServerIP);
...
const BOOL ServiceManager::AcceptThread()
{
	...
	while ( m_AcceptThread.IsRun() )
	{
		...
		wsprintf(log,"[%d] New Client Access : %d",hClntSock ,++i);
		m_Log.EventLog(1, log);
